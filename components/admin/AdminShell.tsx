  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-50 lg:grid lg:grid-cols-[17rem_1fr]">
      <aside className="border-b border-slate-400/10 bg-slate-950/95 px-5 py-5 lg:min-h-screen lg:border-b-0 lg:border-r lg:px-4">
        <div className="flex items-center gap-3 rounded-2xl border border-cyan-300/15 bg-cyan-300/10 p-4">
          <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-cyan-300 text-slate-950">
            <Sparkles aria-hidden="true" className="h-5 w-5" />
          </div>
          <div>
            <p className="text-sm font-semibold text-slate-50">Portfolio Admin</p>
            <p className="text-xs text-slate-400">{session.email}</p>
          </div>
        </div>

        <nav className="mt-5 flex gap-2 overflow-x-auto pb-2 lg:block lg:space-y-1 lg:overflow-visible lg:pb-0" aria-label="Admin navigation">
          {navigation.map((item) => {
            ...
            return (
              <Link
                key={item.href}
                href={item.href}
                className="inline-flex min-w-max items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium text-slate-300 transition hover:bg-slate-900 hover:text-cyan-200 focus:outline-none focus:ring-2 focus:ring-cyan-300 lg:flex lg:min-w-0"
              >
        ...
        <header className="sticky top-0 z-10 border-b border-slate-400/10 bg-slate-950/85 px-5 py-4 backdrop-blur lg:px-8">
          <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <h1 className="text-xl font-semibold text-slate-50">Portfolio Control Center</h1>
            </div>
            <Link href="/" className="text-sm font-medium text-cyan-200 hover:text-cyan-100 focus:outline-none focus:ring-2 focus:ring-cyan-300">
              View Site
            </Link>
          </div>
        </header>
