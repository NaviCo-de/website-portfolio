import { Inbox, Share2, WalletCards, FolderKanban } from "lucide-react";
...
export default async function AdminDashboardPage() {
  ...
  return (
    <div>
      <PageHeader title="Dashboard" description="Quick overview of portfolio content and incoming contact messages." />
      ...
      <section className={`${panelClass} mt-6`}>
        ...
        <div className="mt-5 grid gap-4 md:grid-cols-2">
          <div className="rounded-xl border border-slate-500/15 bg-slate-950/50 p-4">
            <p className="text-sm font-medium text-slate-300">Tanggal update terakhir</p>
            <p className="mt-2 text-sm text-slate-500">
              {data.latestSettings?.updatedAt ? new Intl.DateTimeFormat("en", { dateStyle: "medium", timeStyle: "short" }).format(data.latestSettings.updatedAt) : "No settings update yet."}
            </p>
          </div>
          <div className="rounded-xl border border-slate-500/15 bg-slate-950/50 p-4">
            <p className="text-sm font-medium text-slate-300">Latest Message</p>
            {data.latestMessage ? (
              <p className="mt-2 text-sm leading-6 text-slate-500">{data.latestMessage.subject} from {data.latestMessage.email}</p>
            ) : (
              <p className="mt-2 text-sm text-slate-500">No messages received yet.</p>
            )}
          </div>
        </div>
      </section>
