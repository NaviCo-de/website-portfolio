...
export const contactSchema = z.object({
  ...
  subject: z.string().trim().min(1).max(160).transform((value) => sanitizeText(value, 160)),
  message: z.string().trim().min(1).max(5000).transform((value) => sanitizeText(value, 5000)),
  website: z.string().optional().default(""),
});

export const profileSchema = z.object({
  name: z.string().trim().min(1).max(120).transform((value) => sanitizeText(value, 120)),
...
export const aboutSchema = z.object({
  title: z.string().trim().min(1).max(120).transform((value) => sanitizeText(value, 120)),
  subtitle: z.string().optional().transform(emptyToNull),
  description: z.string().trim().min(1).max(5000).transform((value) => sanitizeText(value, 5000)),
});

export const experienceSchema = z.object({
  position: z.string().trim().min(1).max(140).transform((value) => sanitizeText(value, 140)),
  organization: z.string().trim().min(1).max(160).transform((value) => sanitizeText(value, 160)),
...
export const projectSchema = z.object({
  title: z.string().trim().min(1).max(160).transform((value) => sanitizeText(value, 160)),
  description: z.string().trim().min(1).max(5000).transform((value) => sanitizeText(value, 5000)),
...
export const socialLinkSchema = z.object({
  platform: z.string().trim().min(1).max(80).transform((value) => sanitizeText(value, 80)),
...
export const settingsSchema = z.object({
  siteTitle: z.string().trim().min(1).max(160).transform((value) => sanitizeText(value, 160)),
